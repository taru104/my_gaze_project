"""exp22: exp21の勝ちパターンを実用化。7D(近距離精度)×眼球モデル(距離ロバスト)を距離で滑らかブレンド。
各点の距離変化 rel=|dist-calib_dist|/calib_dist に応じ、w=clip((rel-0.05)/0.10,0,1)で
pred = (1-w)*7D予測 + w*眼球予測。予測値同士の重み付き平均=滑らか(ハード切替でないので破綻しない)。
距離bin別に 7D単独 / 眼球単独 / ブレンド を比較。ブレンドが全距離で最良なら実用形として有望。mainは触らない。
"""
import sys, glob
...
def predict(model, Xte):
    sc, ms=model
    return np.stack([ms[a].predict(sc.transform(Xte)) for a in range(2)], axis=1)
...
for binp in sorted(glob.glob(str(ROOT/"logs"/"*_landmarks.bin"))):
    ...
    for k in idx:
        ...
        f7,yaw,_,_=r7
        pts.append(dict(eye=gz/ng, f7=np.asarray(f7,float), tgt=np.asarray(t,float),
...
log(f"\n---\n## exp22: 7D×眼球 距離ブレンド（{len(sessions)}セッション, honest）")
res={b:{"7d":[],"eye":[],"blend":[]} for b in DBINS}
for pts in sessions:
    ...
    X7=np.array([pts[i]["f7"] for i in tr]); Xe=np.array([pts[i]["eye"] for i in tr]); Y=np.array([pts[i]["tgt"] for i in tr])
    m7=fit_model(X7,Y); me=fit_model(Xe,Y)
    for b in DBINS:
        lo,hi=b
        ...
        pe=predict(me, np.array([pts[i]["eye"] for i in te]))
        Yte=np.array([pts[i]["tgt"] for i in te])
        w=np.clip((rel[te]-0.05)/0.10, 0.0, 1.0)[:,None]
        pb=(1-w)*p7 + w*pe
        res[b]["7d"]+=list(euc(p7,Yte)); res[b]["eye"]+=list(euc(pe,Yte)); res[b]["blend"]+=list(euc(pb,Yte))
log(f"\n**距離bin別, median cm（学習=キャリブ距離±5%）**")
log(f"  {'距離変化':>10} | {'7D単独':>8} | {'眼球単独':>8} | {'ブレンド':>8}")
for b in DBINS:
    r=res[b]
    if r["7d"] and r["eye"] and r["blend"]:
        log(f"  {names[b]:>10} | {np.median(r['7d']):>6.2f}cm | {np.median(r['eye']):>6.2f}cm | {np.median(r['blend']):>6.2f}cm")
allb=[v for b in DBINS for v in res[b]["blend"]]; all7=[v for b in DBINS for v in res[b]["7d"]]
if allb:
    log(f"\n- 全体: 7D単独={np.median(all7):.2f}cm / ブレンド={np.median(allb):.2f}cm")
    log("- ブレンドが各距離binで単独最良以上なら、7Dの近距離精度と眼球の距離ロバストを両取り＝実用形の核。")
